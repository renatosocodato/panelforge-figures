...
| 3 | Pass-3 LLM bridge skipped, fields unmapped. | `ANTHROPIC_API_KEY` not set, or `panelforge-figures[claude-autonomous]` extras not installed. | Either install extras and export the key, or accept the unmapped-fields list and rename CSV columns to match the contract field names exactly. |
