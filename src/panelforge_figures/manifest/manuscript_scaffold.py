...
# --------------------------------------------------------------------------- #
# E10 — Line-anchored insertion helper                                        #
# --------------------------------------------------------------------------- #


def insert_blocks_into_existing(
    existing_text: str,
    insertions: list[dict],
    *,
    format: ManuscriptFormat = ManuscriptFormat.latex,
) -> str:
    """Apply line-anchored insertions to an existing manuscript text.

    Each insertion is a dict ``{"line": int, "content": str}`` where
    ``line`` is the 1-indexed line number **in the original** text and
    ``content`` is the block to insert *before* that line.

    Multiple insertions are processed in ascending ``line`` order; the
    cumulative offset shift caused by previous insertions is tracked
    automatically so callers can describe insertions in terms of the
    original line numbers (rather than having to re-compute offsets).

    Parameters
    ----------
    existing_text
        The original manuscript text (LaTeX or Markdown).
    insertions
        List of ``{"line": int, "content": str}`` dicts.  Each block is
        inserted **before** the specified line.  ``line=N`` where N
        exceeds the number of lines is treated as "append at end".
    format
        Format hint — only used for the inter-block separator (LaTeX
        gets ``\n\n``, Markdown gets ``\n\n`` as well; the parameter is
        present for forward compatibility with rich-format diffs).

    Returns
    -------
    str
        The modified text with all insertions applied.  Trailing
        newline preserved if present in the original.
    """
    if not existing_text:
        # Empty original — concatenate inserted blocks in order.
        sorted_inserts = sorted(
            insertions, key=lambda d: int(d.get("line", 0))
        )
        sep = "\n\n"
        return sep.join(str(d.get("content", "")) for d in sorted_inserts)
    ...
    # Sort by line number ascending so cumulative shift is monotone.
    sorted_inserts = sorted(
        insertions, key=lambda d: int(d.get("line", 0))
    )

    # Build (insertion_index, content_lines) pairs.
    expanded: list[tuple[int, list[str]]] = []
    for ins in sorted_inserts:
        line_num = int(ins.get("line", 1))
        # Clamp to [1, len(lines)+1].
        line_num = max(1, min(line_num, len(lines) + 1))
        content = str(ins.get("content", ""))
        content_lines = content.splitlines() if content else [""]
        expanded.append((line_num, content_lines))

    # Apply insertions, walking the original lines and tracking offsets.
    out_lines: list[str] = []
    insert_idx = 0
    for i, line in enumerate(lines, start=1):
        # Drain any insertions whose target line is i.
        while insert_idx < len(expanded) and expanded[insert_idx][0] == i:
            out_lines.extend(expanded[insert_idx][1])
            # Add a blank separator line if the inserted block doesn't
            # already end in one — keeps the inserted figure block from
    ...
    # Any leftover insertions target lines past the end → append them.
    while insert_idx < len(expanded):
        if out_lines and out_lines[-1].strip():
            out_lines.append("")
        out_lines.extend(expanded[insert_idx][1])
        insert_idx += 1
