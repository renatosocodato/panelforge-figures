"""Cue-to-net-displacement latency (tau_drift) — split-violin per
condition.

How long after cue onset does the cell's net projected
displacement along the cue direction exceed a threshold for a
sustained window?
...
class CueToNetDisplacementInput(RecipeContract):
    latencies: list[LatencyDistribution] = Field(..., min_length=2)
    drift_window_s: float = 60.0
    min_projection_um: float = 3.0
    log_y: bool = False
    title: str = "Cue -> net displacement latency (tau_drift)"


def _demo() -> CueToNetDisplacementInput:
    rng = np.random.default_rng(2811)
    return CueToNetDisplacementInput(
        latencies=[
            LatencyDistribution(
                label="tau_drift", condition="control",
                values_s=rng.gamma(shape=3.0, scale=6.0, size=70).tolist(),
                n_subjects=70,
            ),
            LatencyDistribution(
                label="tau_drift", condition="DISC1",
                values_s=rng.gamma(shape=2.5, scale=12.0, size=80).tolist(),
                n_subjects=80,
            ),
        ],
    )
...
_META = RecipeMetadata(
    name="cue_to_net_displacement_latency",
    modality="intravital_imaging",
    family=RecipeFamily.split_violin,
    answers_question=(
        "How long after cue onset does the cell's net projected "
        "displacement along the cue exceed a threshold for a "
        "sustained window?"
    ),
    required_fields=("latencies",),
    optional_fields=(
        "drift_window_s", "min_projection_um", "log_y", "title",
    ),
...
@register_recipe(
    metadata=_META,
    contract=CueToNetDisplacementInput,
    demo_contract=_demo,
)
def render(contract: CueToNetDisplacementInput, ax=None, **_):
    if ax is None:
    ...
    ax.set_title(
        f"{contract.title}  ·  >= "
        f"{smart_fmt(contract.min_projection_um)} um for "
        f"{smart_fmt(contract.drift_window_s)} s  ·  "
